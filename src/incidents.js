...
function createIncidentsRepo(db) {
  const statements = {
    insert: db.prepare(
      `INSERT INTO incidents (service_id, started_at, ended_at, duration_seconds, status_code, error_message, check_count)
       VALUES (@service_id, @started_at, @ended_at, @duration_seconds, @status_code, @error_message, @check_count)`
    ),
  ...
  return {
    ...
    recordFailure(id, { status_code, error_message, duration_seconds, check_count }) {
